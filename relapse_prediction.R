...
# DIFENG ------------------------------------------------------------------
# Subset of dataset
# col_index <- c(sample(1:750, 74), 751:824)
# sample_mile <- mile_data[col_index]
# yeoh_index <- c(1:74, 211:284)
# sample_yeoh <- yeoh_data[yeoh_index]
...
ttest_pvalue <- calc_ttest(qnorm_yeoh, 210)
log_fc <- calc_logfc(qnorm_yeoh[,1:210], qnorm_yeoh[,-(1:210)])
pvalue_probesets <- names(ttest_pvalue)[ttest_pvalue <= 0.01]
...
# RETROSPECTIVE -----------------------------------------------------------
results_difeng <- read.table("dump/results-difeng.tsv", sep = "\t", header = T, row.names = 1)
results_yeoh <- read.table("dump/results-yeoh.tsv", sep = "\t", header = T, row.names = 1)
results_gfs <- read.table("dump/results-gfs.tsv", sep = "\t", header = T, row.names = 1)
labels_vec <- results_difeng[order(rownames(results_difeng)), 4]
difeng_vec <- results_difeng[order(rownames(results_difeng)), 1]
yeoh_vec <- results_yeoh[order(rownames(results_difeng)), 1]
gfs_vec <- results_gfs[order(rownames(results_difeng)), 1]

par(mar = rep(5,4))
plot_roc(list(difeng_vec, yeoh_vec, gfs_vec), labels_vec,
         name_vec = c("Quantile - PCA", "Quantile - Probesets", "GFS - PCA"))
results_roc <- recordPlot()
save_fig(results_roc, "dump/results-default.pdf",
         width = 9, height = 9)
