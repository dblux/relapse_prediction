...
### UNIT TESTING ###
test_that("predict_pipeline_v2 estimates the correct direction for all features", {
  prediction <- predict_pipeline_v2(
    X_subtype, X_normal, metadata, metadata_pid, batch_ps
  )
  p_sub <- prediction$p_remission[
    metadata_pid[rownames(prediction$p_remission), "label"] == 0,
  ]
  X_y_sub <- prediction$X_y[
    metadata_pid[rownames(prediction$X_y), "label"] == 0,
  ]
  
  expect_equal(
    rank(p_sub$erm1_ratio2),
    rank(X_y_sub$erm1_ratio2) 
  )
  expect_equal(
    rank(p_sub$l2norm_ratio2),
    rank(X_y_sub$l2norm_ratio2) 
  )
  expect_equal(
    rank(-p_sub$angle_d0d8_d0normal),
    rank(X_y_sub$angle_d0d8_d0normal) 
  )
  expect_equal(
    rank(-p_sub$log_mrd),
    rank(X_y_sub$log_mrd) 
  )
})
