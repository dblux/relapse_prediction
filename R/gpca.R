...
gPCA.batchdetect <- function(
  ...
  msg	<- paste0(
    "Missing values detected. Continue with mean value imputation? ",
    "(Note this may take a very long time, but it will automatically ",
    "save in your working dir so you don't have to ever run it again.) [y/n]"
  )
  if(sum(is.na(x))>0) { 
    missing<-readline(prompt=msg)
